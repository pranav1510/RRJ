...
public interface PaymentInfoRepository extends JpaRepository<PaymentInfo,Integer> {
    ...
    @Query("select e FROM PaymentInfo e WHERE e.PaymentId =?1 OR e.CustomerFullName =?2 OR e.CustomerMobile =?3 OR e.PaymentDate =?4 OR e.PaymentReceivedBy =?5 ORDER BY e.PaymentId DESC")
    public List<PaymentInfo> getdetails(String PaymentId, String CustomerFullName, String CustomerMobile, String PaymentDate, String PaymentReceivedBy);
